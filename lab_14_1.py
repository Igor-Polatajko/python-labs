...
class ToDoItem:
    ...
    def __init__(self, item_id, title, content, priority):
    ...
    def db_read(method):
        def wrapper(*args, **kwargs):
            ...
            return [ToDoItem(result[0], result[1], result[2], result[3]) for result in results]
...
class ToDoItemDao:
    ...
    def create(self, item):
        self.db.cursor().execute("INSERT INTO todo_items (title, content, priority, completed)"
                                 " VALUES (?, ?, ?, ?)", (item.title, item.content, item.priority, item.priority))
        self.db.commit()
...
class View:
    ...
    def show_help(self):
        print('\n################ Commands ####################\n'
              ...
              'check item_id - mark as finished\n'
              'uncheck item_id - unmark finished\n'
    ...
    def item_view(self, item=None):
        ...
        return ToDoItem(None, title, content, ITEM_PRIORITY[priority])
...
class ViewHandler:
    ...
    def handle(self, user_input):
        ...
        if i[0] == 'exit':
            ...
            if len(i) > 1:
                item = self._todo_dao.find_by_field('id', i[1])
                if item:
                    updated_item = self._view.item_view(item[0])
                    updated_item.item_id = item[0].item_id
